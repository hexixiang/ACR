...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.total_collapsed_groups = 0 # changed
        self.total_collapsed_groups_ones = 0 # changed
        self.total_collapsed_groups_zeros = 0 # changed
        self.total_groups_seen = 0 # changed
    ...
    def _generate_and_score_completions(
        ...
        # print("prompt_text[0]:",prompts_text[0]) #changed
        ...
        num_groups_in_batch = grouped_rewards.size(0)

        if num_groups_in_batch > 0:
            # 2. 计算通用塌缩率 (组内奖励标准差为0)
            std_per_group = grouped_rewards.std(dim=1)
            collapsed_groups_general = torch.isclose(std_per_group, torch.zeros_like(std_per_group)).sum().item()

            # 3. 计算特定原因导致的塌缩: 全0奖励和全1奖励
            collapsed_groups_zeros = (grouped_rewards == 0).all(dim=1).sum().item()
            collapsed_groups_ones = (grouped_rewards == 1).all(dim=1).sum().item()

            # 4. 计算当前批次的各种塌缩率
            batch_rate_general = collapsed_groups_general / num_groups_in_batch
            batch_rate_zeros = collapsed_groups_zeros / num_groups_in_batch
            batch_rate_ones = collapsed_groups_ones / num_groups_in_batch
            
            # 5. 更新累计计数器
            self.total_groups_seen += num_groups_in_batch
            self.total_collapsed_groups += collapsed_groups_general
            self.total_collapsed_groups_zeros += collapsed_groups_zeros
            self.total_collapsed_groups_ones += collapsed_groups_ones

            # 6. 计算累计塌缩率
            cumulative_rate_general = self.total_collapsed_groups / self.total_groups_seen
            cumulative_rate_zeros = self.total_collapsed_groups_zeros / self.total_groups_seen
            cumulative_rate_ones = self.total_collapsed_groups_ones / self.total_groups_seen
        else:
            # 处理批次中组数为0的边缘情况
            batch_rate_general, batch_rate_zeros, batch_rate_ones = 0.0, 0.0, 0.0
            if self.total_groups_seen > 0:
                cumulative_rate_general = self.total_collapsed_groups / self.total_groups_seen
                cumulative_rate_zeros = self.total_collapsed_groups_zeros / self.total_groups_seen
                cumulative_rate_ones = self.total_collapsed_groups_ones / self.total_groups_seen
            else:
                cumulative_rate_general, cumulative_rate_zeros, cumulative_rate_ones = 0.0, 0.0, 0.0

        # 7. 将所有新指标添加到日志字典中
        # 批次指标
        self._metrics[mode]["metrics/collapse_rate/general_batch"].append(batch_rate_general)
        self._metrics[mode]["metrics/collapse_rate/zeros_batch"].append(batch_rate_zeros)
        self._metrics[mode]["metrics/collapse_rate/ones_batch"].append(batch_rate_ones)
        # 累计指标
        self._metrics[mode]["metrics/collapse_rate/general_cumulative"].append(cumulative_rate_general)
        self._metrics[mode]["metrics/collapse_rate/zeros_cumulative"].append(cumulative_rate_zeros)
        self._metrics[mode]["metrics/collapse_rate/ones_cumulative"].append(cumulative_rate_ones)
    
        # changed:计算优势塌缩率


        if self.accelerator.is_main_process: # changed
        ...
        # Changed: add the virtual advantages to recompute the advantages
        # 如果奖励全0，则增加一个1的奖励，如果奖励全1，则增加一个0的奖励，其他情况不变
        need_virtual_sample = torch.all(rewards == 0) or torch.all(rewards == 1)

        if need_virtual_sample:
            if torch.all(rewards == 0):
                virtual_reward_value = 1.0
            else:  # torch.all(rewards == 1)
                virtual_reward_value = 0.0
            
            # 为每个组添加一个虚拟样本
            num_groups = rewards.shape[0] // self.num_generations
            virtual_samples = torch.full((num_groups,), virtual_reward_value, device=rewards.device)
            
            # 重新组织数据：每组原有样本 + 1个虚拟样本
            virtual_rewards_list = []
            for i in range(num_groups):
                start_idx = i * self.num_generations
                end_idx = (i + 1) * self.num_generations
                group_rewards = rewards[start_idx:end_idx]
                group_with_virtual = torch.cat([group_rewards, virtual_samples[i:i+1]])
                virtual_rewards_list.append(group_with_virtual)
            
            virtual_rewards = torch.cat(virtual_rewards_list)
            
            # 计算包含虚拟样本的优势值
            virtual_mean_grouped_rewards = virtual_rewards.view(-1, self.num_generations + 1).mean(dim=1)
            virtual_mean_grouped_rewards = virtual_mean_grouped_rewards.repeat_interleave(self.num_generations + 1, dim=0)
            virtual_advantages = virtual_rewards - virtual_mean_grouped_rewards
            
            # 提取原始样本对应的优势值（去掉虚拟样本的优势值）
            advantages_list = []
            for i in range(num_groups):
                start_idx = i * (self.num_generations + 1)
                end_idx = start_idx + self.num_generations  # 只取原始样本的优势值
                advantages_list.append(virtual_advantages[start_idx:end_idx])
        ...
        if self.scale_rewards in ["group", "none"]:
            if need_virtual_sample:
                # 使用虚拟样本计算标准差，但只保留原始样本对应的部分
                virtual_std_rewards = virtual_rewards.view(-1, self.num_generations + 1).std(dim=1)
                virtual_std_rewards = virtual_std_rewards.repeat_interleave(self.num_generations, dim=0)  # 注意这里是num_generations
                std_rewards = virtual_std_rewards
            else:
            ...
            if need_virtual_sample:
                # 使用虚拟样本计算全局标准差
                virtual_std_value = virtual_rewards.std()
                std_rewards = virtual_std_value.expand_as(rewards)  # 注意这里expand_as(rewards)而不是virtual_rewards
            else:
        ...
        # Calculate advantage variance for analysis (changed)
        advantage_variance_grouped = advantages.view(-1, self.num_generations).var(dim=1)
        self._metrics[mode]["advantages/variance_mean"].append(advantage_variance_grouped.mean().item())
        self._metrics[mode]["advantages/variance_std"].append(advantage_variance_grouped.std().item())
